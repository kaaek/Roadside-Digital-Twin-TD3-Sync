...
class Z3FeasibilityChecker:
    ...
    def _build_snapshot(self, scenario: Scenario) -> ScenarioVerificationSnapshot:
        ...
        active_mask_time_matrix = np.zeros((horizon + 1, pair_count), dtype=bool)
        ...
        for time_slot_index in range(horizon + 1):
            ...
            for pair in scenario.sensor_pair_index.pairs:
                ...
                active_mask_time_matrix[time_slot_index, pair_index] = bool(
                    in_zone_vehicle_mask[vehicle_index]
                )
        ...
        return ScenarioVerificationSnapshot(
            active_pair_mask_time_matrix=active_mask_time_matrix,
            ...
            priority_weight_array=scenario.priority_weight_array_by_pair(),
    ...
    def compute_strictness_diagnostics(self) -> StrictnessDiagnostics:
        ...
        active_mask = self.snapshot.active_pair_mask_time_matrix[:horizon]
        active_counts = np.sum(active_mask, axis=1)
        schedulable_mask = (
            active_mask
            & (
        ...
        if window_length > 0:
            for start_index in range(0, horizon + 2 - window_length):
                window_mask = self.snapshot.active_pair_mask_time_matrix[
                    start_index : start_index + window_length
                ]
                persistent_pair_count_list.append(int(np.sum(np.all(window_mask, axis=0))))
        ...
        return StrictnessDiagnostics(
            ...
            maximum_active_pair_count=int(np.max(active_counts)) if active_counts.size else 0,
            mean_active_pair_count=float(np.mean(active_counts)) if active_counts.size else 0.0,
    ...
    def _build_z3_problem(
        ...
        a = [[z3.Real(f"A_{time_index}_{pair_index}") for pair_index in range(pair_count)] for time_index in range(horizon + 1)]
        c = [z3.Real(f"C_{time_index}") for time_index in range(horizon + 1)]
        for pair_index in range(pair_count):
            z3_context.add(a[0][pair_index] == self._real_value(z3, 1.0))
        ...
        for time_index in range(horizon):
            ...
            for pair_index in range(pair_count):
                if pair_index not in set(active_indices):
            ...
            for pair_index in range(pair_count):
                ...
                z3_context.add(
                    ...
                    == z3.If(
                        ...
                        a[time_index][pair_index] + self._real_value(z3, 1.0),
                    )
                )
        if enforce_freshness_boolean:
            for time_index in range(horizon + 1):
                for pair_index in np.where(self.snapshot.active_pair_mask_time_matrix[time_index])[0].astype(int).tolist():
                    z3_context.add(a[time_index][pair_index] <= self._real_value(z3, tau))
        for time_index in range(horizon + 1):
    ...
    def _build_objective_expression(self, z3, aoi_variables: list[list[Any]]) -> Any:
        ...
        for time_index in range(1, horizon + 1):
            active_indices = np.where(self.snapshot.active_pair_mask_time_matrix[time_index])[0].astype(int).tolist()
            for pair_index in active_indices:
                weight = float(self.snapshot.priority_weight_array[pair_index])
                weighted_terms.append(self._real_value(z3, weight) * aoi_variables[time_index][pair_index])
    ...
    def _build_note_string(self, status_string: str, diagnostics: StrictnessDiagnostics) -> str:
        ...
        if diagnostics.persistent_pair_capacity_violation_boolean:
            return (
                "The constraints are likely too strict: more sensor-vehicle pairs remain active across a freshness window "
